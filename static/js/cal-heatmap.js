...
var CalHeatMap = function() {
  ...
  function _initCalendar() {
    ...
    self.Legend.redraw(
      self.graphDim.width - self.options.domainGutter - self.options.cellPadding
    )
    self.afterLoad()
   ...
   */
  this.paint = function(navigationDir = 1) {
    ...
    rect
      ...
      .on('click', function(d) {
        if (options.onClick !== null) {
          return self.onClick(new Date(this.__data__.t), this.__data__.v)
        }
      })
        ...
        if (options.tooltip) {
          selection.on('mouseover', function() {
            ...
            var tooltipPositionX =
              self.positionSubDomainX(d.t) -
              self.tooltip._groups[0][0].offsetWidth / 2 +
              options.cellSize / 2
            ...
            tooltipPositionX += parseInt(
              self.root.select('.graph').attr('x'),
              10
            )
            tooltipPositionY += parseInt(
              self.root.select('.graph').attr('y'),
              10
            )
    ...
    if (!options.tooltip) {
      rect.append('title').text(function() {
        return self.formatDate(
          new Date(this.__data__.t),
    ...
    if (options.subDomainTextFormat !== null) {
      rect
        .append('text')
        .attr('class', function(d) {
          return 'subdomain-text' + self.getHighlightClassName(this.__data__.t)
        })
        .attr('x', function(d) {
          return self.positionSubDomainX(this.__data__.t) + options.cellSize / 2
        })
        .attr('y', function(d) {
          return self.positionSubDomainY(this.__data__.t) + options.cellSize / 2
        })
...
CalHeatMap.prototype = {
  ...
  init: function(settings) {
    ...
    if (d3.select(options.itemSelector)._groups[0][0] === null) {
      throw new Error(
        "The node '" +
  ...
  fill: function(svg) {
    ...
    rect
      ...
      .select('rect')
      .attr('class', function(d) {
        var htmlClass = parent
          .getHighlightClassName(this.__data__.t)
          .trim()
          .split(' ')
        var pastDate = parent.dateIsLessThan(this.__data__.t, new Date())
        var sameDate = parent.dateIsEqual(this.__data__.t, new Date())
    ...
    function formatSubDomainText(element) {
      if (typeof options.subDomainTextFormat === 'function') {
        element.text(function(d) {
          return options.subDomainTextFormat(this.__data__.t, this.__data__.v)
        })
      }
    }
  ...
  getDomainKeys: function() {
    'use strict'
    const keys = Array.from(this._domains.keys())
  ...
  setLegend: function() {
    ...
    this.Legend.redraw(
      this.graphDim.width - this.options.domainGutter - this.options.cellPadding
    )
  },
  ...
  getSVG: function() {
    ...
    var getElement = function(e) {
      return root.select(e)._groups[0][0]
    }
...
DomainPosition.prototype.shiftRightBy = function(exitingDomainDim) {
  ...
  this.positions.forEach((key, value) => {
    this.positions.set(key, value - exitingDomainDim)
  })
...
DomainPosition.prototype.shiftLeftBy = function(enteringDomainDim) {
  ...
  this.positions.forEach((key, value) => {
    this.positions.set(key, value + enteringDomainDim)
  })
...
Legend.prototype.redraw = function(width) {
  ...
  if (legendElement._groups[0][0] !== null) {
...
Legend.prototype.buildColors = function() {
  ...
  this.calendar.legendScale = d3.scaleThreshold()
